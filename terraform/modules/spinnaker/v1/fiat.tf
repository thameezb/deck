resource "kubernetes_deployment" "fiat" {
    ...
    template {
      metadata {
        labels = merge({
          ...
          "app.kubernetes.io/version"    = "1.28.1"
          "cluster"                      = "spin-fiat"
        }, module.config["spin-fiat"].labels)
        annotations = {}
      }
      ...
      spec {
        ...
        volume {
          ...
          config_map {
            default_mode = "0644"
            name         = module.config["spin-fiat"].name
          }
        }
        volume {
          name = "empty-dir"
...
resource "kubernetes_service" "fiat" {
  metadata {
    name      = "spin-fiat"
    namespace = local.namespace
    annotations = {
  ...
  spec {
    ...
    selector = {
      app     = kubernetes_deployment.fiat.spec[0].selector[0].match_labels.app
      cluster = kubernetes_deployment.fiat.spec[0].selector[0].match_labels.cluster
    }
  }
